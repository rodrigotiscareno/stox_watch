...
def fetch_income_statements(symbol):
    url = (
        f"https://www.alphavantage.co/query?function=INCOME_STATEMENT&symbol={symbol}&apikey={api_key}"
    )
...
def fetch_and_save_income_statements():
    ...
    #csv file name
    csv_file = "../notebooks/data/income_statements_data.csv"
    # Save DataFrame to CSV
    df.to_csv(csv_file, index=False)

    load_csv_to_sql("../notebooks/data/income_statements_data.csv", "inc_stmts")
