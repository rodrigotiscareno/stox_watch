...
def browse_page():
    ...
    min_price = forecasted_price_df["forecasted_price"].min()
    max_price = forecasted_price_df["forecasted_price"].max()
    forecast_chart = (
        alt.Chart(forecasted_price_df)
        .mark_line()
        .encode(
            x=alt.X("date:T", title="Date", axis=alt.Axis(tickCount="day")),
            y=alt.Y(
                "forecasted_price:Q",
                title="Forecasted Price",
                scale=alt.Scale(domain=[min_price * 0.98, max_price * 1.02]),
                axis=alt.Axis(tickCount=10),
            ),
            tooltip=["date:T", "forecasted_price:Q"],
        )
        .properties(
            width=800, height=300, title=f"Forecasted Price for {selected_ticker}"
        )
    )
    ...
    chart = (
        ...
        .properties(
            width=800, height=400, title=f"Sentiment Heatmap for {selected_ticker}"
        )
    )
    st.altair_chart(chart, use_container_width=True)
